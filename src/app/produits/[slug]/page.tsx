...
export async function generateMetadata(props: {
  ...
  if (!product) {
    return {
      title: 'Produit non trouvé | Just Hype',
  ...
  const imageUrl = productImage
    ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/products/${productImage}`
    : null

  const description = product.description 
    ? product.description.substring(0, 160) 
    : `${product.name} - ${product.price_after_discount || product.price} TND | Just Hype`
  ...
  return {
    title: `${product.name} | Just Hype`,
    description,
    keywords: [product.name, 'Just Hype', 'mode', 'homme', 'Tunisie', categoryName || ''].filter(Boolean),
    openGraph: {
      title: product.name,
      description: `${product.name} - ${product.price_after_discount || product.price} TND`,
      type: 'website',
      locale: 'fr_TN',
    ...
    alternates: {
      canonical: `${SITE_URL}/produits/${slug}`,
    },
  }
}
...
export default async function ProductPage(props: {
  ...
  const productImage = typedProduct.product_images?.[0]?.image_path
  const imageUrl = productImage
    ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/products/${productImage}`
    : null
  ...
  const structuredData = {
    ...
    description: typedProduct.description || `${typedProduct.name} - Mode masculine premium`,
    ...
    offers: {
      ...
      url: `${SITE_URL}/produits/${slug}`,
      priceCurrency: 'TND',
      price: typedProduct.price_after_discount || typedProduct.price,
